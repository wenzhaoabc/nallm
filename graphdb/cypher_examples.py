def get_fewshot_examples(openai_api_key: str, base_url: str):
    return f"""
...
CALL apoc.ml.openai.embedding(["Are there any news regarding return to office policies?"], 
   "{openai_api_key}",{{"endpoint":{base_url}}}) YIELD embedding
MATCH (c:Chunk)
...
```cypher
CALL apoc.ml.openai.embedding(["What is Microsoft policy regarding to the return to office?"], 
    "{openai_api_key}",{{"endpoint":{base_url}}}) YIELD embedding
